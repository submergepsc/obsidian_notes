...
summary: OI Wiki Markdown 抓取和自包含离线 HTML 归档已完成。
tags:
  - oiwiki
  - crawl
  - obsidian
...
- 更新时间: 2026-04-28 12:57:52 +0800
- 工作目录: `/home/loviya/notes/obsidian_notes/crawl/oiwiki`
- 主机: `nibaba`
- tmux: ``
- 来源指令: `继续爬取oiwiki的工作流`
- 上下文: 未在旧 worklog 中找到直接的 `oiwiki` 记录；根据 `/home/loviya/notes/obsidian_notes/crawl/oiwiki` 中的爬虫和断点创建本工作流。
- 状态: 已完成
- 问题:
  - 目标: 从 `docs/_crawl_state.json` 续爬 OI Wiki，并保证本地 Markdown 内容可用。
  - 阻塞: 无。
- 结果:
...
- 下一步: 无；如未来更新 OI Wiki，可在本目录运行 `.venv/bin/python run_oiwiki.py --output-dir docs` 继续增量抓取，必要时再运行 `--repair-existing`。
- 标签: oiwiki, crawl, obsidian
## 让已完成队列的续爬也能发现 sitemap 中后续新增页面，并执行一次增量检查
- 更新时间: 2026-04-28 14:03:36 +0800
- 工作目录: `/home/loviya/notes/obsidian_notes/crawl/oiwiki`
- 主机: `nibaba`
- tmux: `unavailable`
- 来源指令: `继续爬取oiwiki的工作`
- 上下文: 用户明确要求继续 `oiwiki` 爬取，接续已完成的同一 workflow。
- 状态: 已完成
- 问题:
  - 目标: 让已完成队列的续爬也能发现 sitemap 中后续新增页面，并执行一次增量检查。
  - 阻塞: 无。
- 结果:
...
- 下一步: 无；之后继续运行 `.venv/bin/python run_oiwiki.py --output-dir docs` 即可自动检查 sitemap 新页面。
- 标签: oiwiki, crawl, obsidian
## 仿照菜鸟教程归档，为 OI Wiki 生成 vault 根目录下的离线 HTML 归档和...
- 更新时间: 2026-04-28 14:24:49 +0800
- 工作目录: `/home/loviya/notes/obsidian_notes/crawl/oiwiki`
- 主机: `nibaba`
- tmux: `unavailable`
- 来源指令: `仿照爬取的菜鸟教程的内容,做一个离线 html，并且索引这些都要齐全`
- 上下文: 用户明确要求继续 OI Wiki，并更正目标位置为 `/home/loviya/notes/obsidian_notes` vault 内。
- 状态: 已完成
- 问题:
  - 目标: 仿照菜鸟教程归档，为 OI Wiki 生成 vault 根目录下的离线 HTML 归档和完整索引。
  - 阻塞: 无。
- 结果:
...
- 下一步: 无；如需重建，进入 `crawl/oiwiki` 运行 `.venv/bin/python build_oiwiki_html.py`。
- 标签: oiwiki, crawl, html, obsidian
## 把 vault 根目录的 oiwiki_archive 收进 crawl/oiwiki...
- 更新时间: 2026-04-28 14:31:32 +0800
- 工作目录: `/home/loviya`
- 主机: `nibaba`
- tmux: `unavailable`
- 来源指令: `把~/obnotes/oiwiki_archive和~/obnotes/crawl/oiwiki合并,存到crawl里面`
- 上下文: 继续 OI Wiki 归档路径整理。
- 状态: 已完成
- 问题:
  - 目标: 把 vault 根目录的 `oiwiki_archive` 收进 `crawl/oiwiki` 项目目录。
  - 阻塞: 无。
- 结果:
...
- 下一步: 无；归档入口已改为 `/home/loviya/obnotes/crawl/oiwiki/oiwiki_archive/index.html`。
- 标签: oiwiki, crawl, html, obsidian
## 优化 oiwiki_archive 本地 HTML 阅读和导航体验，并保持可重建
- 更新时间: 2026-04-28 21:29:16 +0800
- 工作目录: `/home/loviya/notes/obsidian_notes/crawl/oiwiki`
- 主机: `nibaba`
- tmux: `/tmp/tmux-1000/default,5441,2`
- 来源指令: `你继续调整~/obnotes/crawl/oiwiki的工作流,优化这个本地网页`
- 上下文: 用户明确要求继续 `~/obnotes/crawl/oiwiki` 的 OI Wiki workflow，接续本文件。
- 状态: 已完成
- 问题:
  - 目标: 优化 `oiwiki_archive` 本地 HTML 阅读和导航体验，并保持可重建。
  - 阻塞: 无。
- 结果:
...
- 下一步: 无；归档入口仍为 `/home/loviya/obnotes/crawl/oiwiki/oiwiki_archive/index.html`，重建命令为 `.venv/bin/python build_oiwiki_html.py`。
- 标签: oiwiki, crawl, html, obsidian, offline-archive
## 保留外站 HTTP/HTTPS 链接，但将所有 oi-wiki.org 站内导航改成本地...
- 更新时间: 2026-04-28 21:39:35 +0800
- 工作目录: `/home/loviya/notes/obsidian_notes/crawl/oiwiki`
- 主机: `nibaba`
- tmux: `/tmp/tmux-1000/default,5441,2`
- 来源指令: `外部的链接仍然作为http的导航留着,所有oiwiki中存在的内容都下载到本地`
- 上下文: 继续 OI Wiki 离线 HTML 本地化调整。
- 状态: 已完成
- 问题:
  - 目标: 保留外站 HTTP/HTTPS 链接，但将所有 `oi-wiki.org` 站内导航改成本地文件，并确认站内内容已抓全。
  - 阻塞: 无。
- 结果:
...
- 下一步: 无；未来重建后可用 `rg -n 'href="https?://oi-wiki\\.org|src="https?://oi-wiki\\.org' oiwiki_archive --glob '*.html' --glob '*.json'` 检查。
- 标签: oiwiki, crawl, html, local-links, offline-archive
## 去掉首页、总索引、全部页面索引中无用的线上 URL 展示文本
- 更新时间: 2026-04-28 21:45:19 +0800
- 工作目录: `/home/loviya/notes/obsidian_notes/crawl/oiwiki`
- 主机: `nibaba`
- tmux: `/tmp/tmux-1000/default,5441,2`
- 来源指令: `index页面存在很多https的显示内容,我根本不会去打开,没必要显示这些,全部删掉或者隐藏`
- 上下文: 继续 OI Wiki 离线 HTML 索引页展示精简。
- 状态: 已完成
- 问题:
  - 目标: 去掉首页、总索引、全部页面索引中无用的线上 URL 展示文本。
  - 阻塞: 无。
- 结果:
...
- 下一步: 无。
- 标签: oiwiki, html, index, local-links
## 左侧悬浮导航支持进入栏目页或原地展开页面；删除单页 subgroup 小节
- 更新时间: 2026-04-28 21:51:47 +0800
- 工作目录: `/home/loviya/notes/obsidian_notes/crawl/oiwiki`
- 主机: `nibaba`
- tmux: `unavailable`
- 来源指令: `左侧的悬浮窗口不是不展开,改成可选是否进去或者展开`
- 上下文: 继续 OI Wiki 离线 HTML 导航体验调整。
- 状态: 已完成
- 问题:
  - 目标: 左侧悬浮导航支持进入栏目页或原地展开页面；删除单页 subgroup 小节。
  - 阻塞: 无。
- 结果:
...
- 下一步: 无。
- 标签: oiwiki, html, sidebar, navigation
