...
- 更新时间: 2026-04-29 12:59:20 +0800
- 工作目录: `/home/loviya`
- 主机: `nibaba`
- tmux: `124922-4680:0.1 /home/loviya`
- 来源指令: `我现在每次打开终端,都会显示(NULL):0: unknown command: tmux,检查一下怎么一回事`
- 上下文: 新建
- 状态: 已完成
- 问题:
  - 目标: 排查并修复每次打开终端都会显示 `(NULL):0: unknown command: tmux` 的问题，并把恢复过程记录下来。
  - 阻塞: 无
- 结果:
  - 已确认问题来自 `~/.tmux.conf` 中 `pane-focus-in` hook 的错误配置，已修复并经用户确认恢复正常。
- 下一步: 无
- 标签: terminal, tmux, bashrc, recovery
#### 错误现象
...
- 更新时间: 2026-04-29 13:45:58 +0800
- 工作目录: `/home/loviya`
- 主机: `nibaba`
- tmux: `124922-4680:0.1 /home/loviya`
- 来源指令: `整理一下`
- 上下文: 继续当前 tmux 配置维护任务。
- 状态: 已完成
- 问题:
  - 目标: 整理 `/home/loviya/.tmux.conf`，去掉重复和被覆盖的配置，保持现有行为不变。
  - 阻塞: 无
- 结果:
...
- 更新时间: 2026-04-29 14:20:38 +0800
- 工作目录: `/home/loviya`
- 主机: `nibaba`
- tmux: `124922-4680:0.1 /home/loviya`
- 来源指令: `我现在在这个pane里面选择的内容,无法复制,我唯一可以复制的方式是shift+鼠标,但是这会选择所有pane里面的内容`
- 上下文: 继续当前 tmux 配置维护任务。
- 状态: 已完成
- 问题:
  - 目标: 修复 tmux pane 内鼠标拖选不能进入系统剪贴板的问题。
  - 阻塞: 无
- 结果:
  - 已将 `copy-mode` 和 `copy-mode-vi` 的 `MouseDragEnd1Pane` 绑定到 `copyq copy -`，避免必须用 `Shift+鼠标` 绕过 tmux。
- 下一步: 用户在 pane 内直接鼠标拖选文本并松开，验证是否能粘贴到系统剪贴板。
- 标签: terminal, tmux, copyq, mouse-copy
#### 修改内容
...
- 更新时间: 2026-04-29 14:28:04 +0800
- 工作目录: `/home/loviya`
- 主机: `nibaba`
- tmux: `124922-4680:0.0 /home/loviya`
- 来源指令: `我查看了,进入了系统剪切板,但是没有进入copyq`
- 上下文: 继续当前 tmux 配置维护任务。
- 状态: 已完成
- 问题:
  - 目标: 让 tmux 复制同时进入系统剪贴板和 CopyQ 历史列表。
  - 阻塞: 无
- 结果:
  - 确认 `copyq copy -` 只写系统剪贴板，不保证进入 CopyQ 历史；已改为 `copyq --start-server add - && copyq --start-server select 0`。
- 下一步: 用户在 pane 内直接鼠标拖选文本并松开，检查 CopyQ 历史和系统剪贴板是否都更新。
- 标签: terminal, tmux, copyq, mouse-copy
#### 修正原因
