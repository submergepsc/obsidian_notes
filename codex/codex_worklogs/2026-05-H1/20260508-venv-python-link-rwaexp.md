...
summary: 已修复 a broken .venv/bin/python chain caused by an unsupported Windows reparse-point symlink and verified plot_3_throught.py runs.
tags:
  - python
  - venv
  - rwaexpresults
...
- 状态: 已完成
- 目标: 解释并fix why `.venv/bin/python` could not be executed for `plot_3_throught.py`.
- 阻塞: 无。
- 下一步: 无。
- 更新时间: 2026-05-08 11:18:00 +0800

## 关键结果

- 已找到 `.venv/bin/python -> python3`, while `.venv/bin/python3` was an unsupported Windows reparse-point symlink.
- Replaced `.venv/bin/python3` 带 a Linux symlink to `/usr/bin/python3.12`.
...
- 已更新 `plot_3_throught.py` so future PoW generation uses the original vertical broken-axis line chart 而不是 the newer boxplot branch.

## Broken Virtualenv Interpreter Link On Windows Mount

- 更新时间: 2026-05-08 11:18:00 +0800
- 工作目录: `/media/windows-c/Users/15056/Desktop/code/RWAExpResults`
- 来源指令: User ran `.venv/bin/python plot_3_throught.py` and Bash reported `没有那个文件或目录`.
- 问题:
...
- 改进:
  - Repaired only the broken interpreter symlink 而不是 clearing or recreating the whole virtual environment.
- 命令:
...
## Restore Throughput Stability PDFs To 之前的 Version

- 更新时间: 2026-05-08 11:22:00 +0800
- 工作目录: `/media/windows-c/Users/15056/Desktop/code/RWAExpResults`
- 来源指令: User asked why the code/results became this way and requested `throughput_s...` images return to the previous version.
- 问题:
...
## Make Plot 3 Regenerate The Original PoW Throughput 图

- 更新时间: 2026-05-08 11:24:00 +0800
- 工作目录: `/media/windows-c/Users/15056/Desktop/code/RWAExpResults`
- 来源指令: User clarified that the PoW figure should be adjusted back to the original by modifying `plot_3_throught.py`.
- 问题:
  ...
  - Regenerating the script would therefore overwrite the restored PoW PDF 带 the wrong chart style again.
- 改进:
  - 已删除 the active PoW boxplot path and restored the original vertical broken-axis line chart path.
  - PoW now uses seconds on the x-axis 带 `k` tick formatting and the old large axis/legend font sizes.
