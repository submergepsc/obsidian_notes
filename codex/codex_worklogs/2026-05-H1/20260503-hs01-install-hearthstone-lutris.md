...
cwd: /home/loviya
summary: 国服炉石已安装；战网启动能进入游戏但匹配阶段在 `libacsdk_x86` 崩溃，已增加直接 EXE 诊断启动器。
tags:
  - lutris
  - hearthstone
  - battle.net
...
# 安装 Hearthstone With Lutris

## 当前快照

- 状态: 进行中
- 目标: 在 Ubuntu 上通过 Wine/Lutris 路径安装并运行炉石传说。
- 阻塞: 国服 Battle.net 启动链路能进入游戏，但匹配阶段被 `libacsdk_x86.dll` 阻塞；直接打开 `Hearthstone.exe` 可用于诊断，但通常无法携带 Battle.net 登录 token。
- 下一步: 运行 `/home/loviya/bin/hearthstone-cn-direct-exe` 验证直启行为；如出现战网登录/token 错误，回到 Battle.net 启动链路或改用 Windows/双系统方案。
- 更新时间: 2026-05-04 05:03:32 +0800
...
- 更新时间: 2026-05-03 12:27:00 +0800
- 工作目录: `/home/loviya`
- 来源指令: `帮我使用lutris安装一下炉石传说`
- 问题:
...
- 更新时间: 2026-05-04 01:21:23 +0800
- 工作目录: `/home/loviya`
- 来源指令: `继续帮我处理在我的ubuntu中玩炉石传说的问题`
- 问题:
  ...
  - 在旧 prefix 中运行新版安装器时，残留 Battle.net Agent 连续返回 `Agent init 返回 status code=401`。
- 改进:
...
- 更新时间: 2026-05-04 04:12:57 +0800
- 工作目录: `/home/loviya`
- 来源指令: `Unhandled exception: page fault ... in libacsdk_x86 ... 运行过程中出错了`
- 问题:
...
- 更新时间: 2026-05-04 04:20:20 +0800
- 工作目录: `/home/loviya`
- 来源指令: `可以打开炉石传说,可以进入游戏,但是进入开始匹配到一般就弹出错误`
- 问题:
...
## 重置 Hearthstone 本地 Cache Before Retest

- 更新时间: 2026-05-04 04:34:00 +0800
- 工作目录: `/home/loviya`
- 来源指令: `还是游戏打开后准备玩,匹配到一般就停止了`
- 问题:
  - 匹配崩溃再次复现：日志路径 `Hearthstone_2026_05_04_04_28_37`，状态从 `BNET_QUEUE_DELAYED` 到 `BNET_QUEUE_UPDATED` 后 `Crash!!!`。
  - `Player.log` 反复记录 `UberText LoadCachedData() failed` 和 `失败 to load config file`，说明本地缓存/选项也有异常信号。
- 改进:
...
## CN Matchmaking 阻塞 By libacsdk_x86

- 更新时间: 2026-05-04 04:35:53 +0800
- 工作目录: `/home/loviya`
- 来源指令: `Unhandled exception: page fault ... in libacsdk_x86 ...`
- 问题:
...
- 更新时间: 2026-05-04 05:03:32 +0800
- 工作目录: `/home/loviya`
- 来源指令: `转换一下工作流,目前我已经安装了国服的炉石传说,能不能用相关软件直接打开这个exe文件`
- 问题:
