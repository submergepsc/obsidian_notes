...
summary: Standardized the five scheme display names across the RWAExpResults plotting scripts and regenerated the figure PDFs.
tags:
  - RWAExpResults
  - plotting
  - scheme-names
...
- status: 已完成
- goal: Use the scheme names from the provided reference figure across the plotting scripts in this session.
- blocker: none
- next: none
- updated: 2026-05-17 17:38:18 +0800

## Key Results

- Standardized scheme labels as:
  - `seenfeed` -> `Sen.[11]`
  - `daon` -> `DAON[12]`
  - `decentruth` -> `DECEN.[13]`
  - `deepthought` -> `Deep.[14]`
  - `committee` -> `FastOracle[15]`
- Updated `plot_1_stacked_bars.py`, `plot_2_queue.py`, `plot_3_throught.py`, `plot_4_certifycate.py`, and `plot_5_scalability.py`.
...
- Fixed the incomplete `LogLocator(base=10.0, subs=(2.0, 5.` line in `plot_2_queue.py` so the plotting suite can run.
...
## Explain Why `pa` Is Unavailable In Current Command Environment

- updated: 2026-05-17 16:02:59 +0800
- cwd: `/home/loviya/code/RWAExpResults`
- source instruction: `当前环境pa为什么没用`
- problem:
...
- result:
  - Use the direct command in non-interactive contexts: `MPLCONFIGDIR=/tmp/matplotlib-cache /home/loviya/code/RWAExpResults/.venv/bin/python /home/loviya/code/RWAExpResults/run_all_plots.py`.
- next:
  - If the user wants `pa` to work everywhere, create a real executable wrapper in `~/.local/bin/pa` instead of relying on shell aliases.

## Remove Citation Numbers From Plot Scheme Labels

- updated: 2026-05-17 16:04:42 +0800
- cwd: `/home/loviya/code/RWAExpResults`
- source instruction: `对了之前修改方案名称的时候,吧中括号和里面的数字删掉`
- problem:
  - The previously standardized labels included citation suffixes such as `[11]` and `[15]`, but the user now wants those removed.
- improvement:
  - Updated the same five plotting scripts so labels are `Sen.`, `DAON`, `DECEN.`, `Deep.`, and `FastOracle`.
...
- updated: 2026-05-17 16:05:35 +0800
- cwd: `/home/loviya/code/RWAExpResults`
- source instruction: `对了plot3还没有修改`
- problem:
  - User reported plot3 may not have been updated.
- improvement:
  - Checked `plot3.py` and `plot_3_throught.py`.
  - Verified generated `figures/03_throughput/throughput_stability_pos.pdf` and `throughput_stability_pow.pdf` with `pdftotext`.
- result:
  - `plot3.py` is an empty 0-line file.
  - The active throughput script `plot_3_throught.py` and both generated throughput PDFs use `FastOracle`, `Deep.`, `Sen.`, `DECEN.`, and `DAON` with no bracketed citation numbers.
- next:
...
## Use Scientific Notation For Plot 4 Ticks

- updated: 2026-05-17 16:15:14 +0800
- cwd: `/home/loviya/code/RWAExpResults`
- source instruction: `不行所有的都是用科学记数法,不需要k`
- problem:
...
- improvement:
  ...
  - Changed the x-axis formatter back to `format_scientific`, so ticks render as `5e3`, `1e4`, and `1.5e4`.
- result:
  - `plot_4_certifycate.py` compiles.
...
## Align Plots 3, 2, And 4 For Three-Column Figure Layout

- updated: 2026-05-17 16:19:13 +0800
- cwd: `/home/loviya/code/RWAExpResults`
- source instruction: `3,2,4这几张图片是放到一起的,你看能否把这几个图片调整一下`
- problem:
...
- improvement:
  - Standardized `plot_2_queue.py`, `plot_3_throught.py`, and `plot_4_certifycate.py` to a shared `8x6` / `576 x 432 pt` PDF page size.
  - Reduced plot2's very large font and marker sizes for a three-column layout.
  - Removed plot4's tight PDF crop so it keeps the same aspect ratio as plot2 and plot3.
...
- result:
  ...
  - Created a temporary POS montage at `/tmp/rwa_pos_324_montage.png` for visual inspection.
- next:
  - none

## Match Plot 2 And Plot 4 Marker Sizes To Plot 5

- updated: 2026-05-17 16:22:13 +0800
- cwd: `/home/loviya/code/RWAExpResults`
- source instruction: `不行,2,4的标记都不行,标点改成5一样大,然后`
- problem:
...
## Fix Plot 4 Annotation Arrow Start And Text Box Size

- updated: 2026-05-17 16:24:32 +0800
- cwd: `/home/loviya/code/RWAExpResults`
- source instruction: `然后plot4中箭头执行的start部分位置不对,文本框也太小了`
- problem:
...
- improvement:
  ...
  - Added text-box edge calculation in display coordinates, then converted that edge point back to data coordinates for the arrow tail.
...
- updated: 2026-05-17 17:01:47 +0800
- cwd: `/home/loviya/code/RWAExpResults`
- source instruction: `这个还记得之前吞吐量的图吗,把plot3的pow改成pos的样式,根据大小的不同,改成`
- problem:
...
- improvement:
  - Added `POW_AXIS_CONFIG` in `plot_3_throught.py` for fixed per-scheme x-axis ranges and tick labels:
...
- updated: 2026-05-17 17:05:50 +0800
- cwd: `/home/loviya/code/RWAExpResults`
- source instruction: `把plot_all的四个文件的所有有关文件的文件结构介绍一下,生成readme文件`
- problem:
...
- improvement:
  - Added `README_plot_all.md` in the project root.
...
- result:
  - README was verified with targeted `rg` and `sed` reads.
- next:
  - none

## Rewrite Project README For Active Plotting Workflow

- updated: 2026-05-17 17:31:17 +0800
- cwd: `/home/loviya/code/RWAExpResults`
- source instruction: `README.md重新写,需要完整介绍整个文件,(不包括实际上很多没有用到或者之前已经废弃的一些文件,),像csv怎么使用的,怎么被文件之间相互调用的,文件之间生成的内容等,结构等,这个readme不是写光一个run_all_plots.py`
- problem:
...
- result:
  - Verified the new README with targeted `rg` and `sed` reads.
- next:
  - none
...
- updated: 2026-05-17 17:38:18 +0800
- cwd: `/home/loviya/code/RWAExpResults`
- source instruction: `对了再次修改一下readme内容,里面涉及到的latex内容不要说,这个main.tex不是我负责的部分,不要当成我自己的`
- problem:
...
- improvement:
  - Removed `main.tex`, `main.pdf`, `figure -> figures`, `figure/...`, and LaTeX reference sections from `README.md`.
...
- result:
  - Verified with `rg` that no `main.tex`, `main.pdf`, `LaTeX`, `figure -> figures`, or `figure/...` references remain in `README.md`.
- next:
  - none

## Standardize Plot Scheme Names

- updated: 2026-05-17 16:00:52 +0800
- cwd: `/home/loviya/code/RWAExpResults`
- source instruction: `整个会话会涉及到4各绘图函数,,每个基本都是五个方案,这无法方案的命名以这个方案为准`
- problem:
  - The plotting scripts used inconsistent old labels such as `Seen.`, `Decen.`, `Daon.`, `Deep.`, and `FastOracle`.
- improvement:
  - Replaced those labels with the citation-bearing names from the provided figure.
  - Included `plot_1_stacked_bars.py` because the reference image is the latency stacked-bar figure.
- result:
