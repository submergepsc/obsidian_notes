...
- Source: https://oi-wiki.org/ds/finger-tree/
...
手指树在树的「手指」（叶子）的地方存储数据，访问时间为分摊常量．手指是一个可以访问部分数据结构的点．在命令式语言（imperative language）中，这被称做指针．在手指树中，「手指」是指向序列末端或叶节点的结构．手指树还在每个内部节点中存储对其后代应用一些关联操作的结果．存储在内部节点中的数据可用于提供除树类数据结构之外的功能．
  ...
  1. 手指树的深度由下到上计算．
  2. 手指树的第一级，即树的叶节点，仅包含值，深度为 00![](data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7)．第二级为深度 11![](data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7)．第三级为深度 22![](data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7)，依此类推．
  3. 离根越近，节点指向的原始树（在它是手指树之前的树）的子树越深．这样，沿着树向下工作就是从叶子到树的根，这与典型的树数据结构相反．为了获得这种的结构，我们必须确保原始树具有统一的深度．在声明节点对象时，必须通过子节点的类型进行参数化．深度为 11![](data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7) 及以上的脊椎上的节点指向树，通过这种参数化，它们可以由嵌套节点表示．

### 将一棵树变成手指树

注释

**2-3 树** 是一种树状数据结构，其中每个带有子节点（内部节点）的节点具有两个子节点（22![](data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7) 节点）和一个数据元素或三个子节点（33![](data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7) 节点）和两个数据元素．2-3 树是 33![](data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7) 阶 B 树．树外部的节点（叶节点）没有子节点和一两个数据元素．
...
```text 1 2 3 4 5 6 ``` |  ```text data FingerTree a = Empty | Single a | Deep ( Digit a ) ( FingerTree ( Node a )) ( Digit a ) data Digit a = One a | Two a a | Three a a a | Four a a a a data Node a = Node2 a a | Node3 a a a ```   
---|---  
...
```text 1 ``` |  ```text type Digit a = One a | Two a a | Three a a a | Four a a a a ```   
---|---  
  
顶层具有类型 𝑎a![](data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7) 的元素，下一层具有类型节点 𝑎a![](data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7) 的元素，因为树脊和叶子之间的节点，这通常意味着树的第 𝑛n![](data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7) 层具有元素类型为 𝑁𝑜𝑑𝑒𝑛Noden![](data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7) 𝑎a![](data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7)，或 2-3 个深度为 𝑛n![](data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7) 的树．这意味着 𝑛n![](data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7) 个元素的序列由深度为 `Θ(log n)` 的树表示．距离最近端 𝑑d![](data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7) 的元素存储在树中 `Θ(log d)` 深度处．
...
```text 1 2 3 4 5 ``` |  ```text data ImplicitDeque a = Empty | Single a | Deep ( Digit a ) ( ImplicitDeque ( a , a )) ( Digit a ) data Digit a = One a | Two a a | Three a a a ```   
---|---  
...
>  __本页面最近更新： 2026/1/7 08:56:54，[更新历史](https://github.com/OI-wiki/OI-wiki/commits/master/docs/ds/finger-tree.md)  
>  __发现错误？想一起完善？[在 GitHub 上编辑此页！](https://oi-wiki.org/edit-landing/?ref=/ds/finger-tree.md "edit.link.title")  
>  __本页面贡献者：[Tiphereth-A](https://github.com/Tiphereth-A), [isdanni](https://github.com/isdanni), [CCXXXI](https://github.com/CCXXXI), [Enter-tainer](https://github.com/Enter-tainer), [ImpleLee](https://github.com/ImpleLee), [Xeonacid](https://github.com/Xeonacid)  
