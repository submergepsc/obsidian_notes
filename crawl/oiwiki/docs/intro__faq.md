...
- Source: https://oi-wiki.org/intro/faq/
...
[![](https://opencollective.com/oi-wiki/contributors.svg?width=890&button=false)](https://github.com/OI-wiki/OI-wiki/graphs/contributors)
...
A：可以通过 [关于本项目里的交流方式](../about/#交流方式) 联系我们．
...
A：在对应页面右上方有一个编辑按钮 _edit_ ，点击并确认阅读了 [如何贡献](../htc/) 之后会跳转到 GitHub 上对应文件的位置．
...
A：有两种选择：
  ...
  * 可以开一个 Pull Request，在目录 [(mkdocs.yml)](https://github.com/OI-wiki/OI-wiki/blob/master/mkdocs.yml) 中加上新的 topic，并在 [docs](https://github.com/OI-wiki/OI-wiki/tree/master/docs) 文件夹下对应位置创建一个空的 `.md` 文件．文档的格式细节请参考 [格式手册](../format/#贡献文档要求)．
...
```text 1 ``` |  ```text git clone https://gitclone.com/github.com/OI-wiki/OI-wiki ```   
---|---  
  
如果您需要向 **OI Wiki** 贡献，那么首先 fork **OI Wiki** 的仓库，然后（将 `username` 替换为您的用户名），需要注意的是提供的示例将使您使用 SSH 连接到 GitHub2：

```text 1 2 ``` |  ```text git clone https://gitclone.com/github.com/username/OI-wiki git remote set-url origin git@github.com:username/OI-wiki.git ```   
---|---  
...
```text 1 ``` |  ```text pip install -U -r requirements.txt -i https://pypi.tuna.tsinghua.edu.cn/simple/ ```   
---|---  
...
```text 1 ``` |  ```text git clone https://github.com/OI-wiki/OI-wiki.git \--depth = 1 -b master ```   
---|---  
...
```text 1 ``` |  ```text python -m pip install \--upgrade pip ```   
---|---  
...
```text 1 ``` |  ```text pip install -U -r requirements.txt ```   
---|---  
...
```text 1 ``` |  ```text 结束位置 <script type="math/tex">endpos</script> ```   
---|---  
  
在目录中会变成

```text 1 ``` |  ```text 结束位置 endpos ```   
---|---  
