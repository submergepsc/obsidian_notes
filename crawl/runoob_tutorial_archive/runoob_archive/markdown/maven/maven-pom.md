...
- Source: https://www.runoob.com/maven/maven-pom.html
...
| 元素 | 说明 | 示例 |
| --- | --- | --- |
| modelVersion | POM 模型版本（固定 4.0.0） | 4.0.0 |
...
| --- | --- | --- | --- | --- |
| 基础信息 |  |  |  |  |
|  | 项目结构 | POM模型版本 | 4.0.0 | 是 |
|  | 坐标 | 组织标识（反向域名） | com.example | 是 |
|  | 坐标 | 项目名称 | my-project | 是 |
|  | 坐标 | 项目版本 | 1.0.0-SNAPSHOT | 是 |
|  | 项目类型 | 打包格式 | jar/war/pom | 否（默认jar） |
|  | 元信息 | 项目显示名称 | My Application | 否 |
|  | 元信息 | 项目描述 | A demo project | 否 |
|  | 元信息 | 项目主页URL | https://example.com | 否 |
| 依赖管理 |  |  |  |  |
|  | 依赖 | 依赖列表容器 | 包含多个 | 否 |
|  | 依赖 | 单个依赖定义 | 包含groupId等子标签 | 可选 |
|  | 依赖 | 依赖作用域 | compile/test/provided/runtime | 否（默认compile） |
|  | 依赖 | 是否可选依赖 | true/false | 否（默认false） |
|  | 依赖 | 排除传递性依赖 | 包含列表 | 否 |
| 构建配置 |  |  |  |  |
|  | 构建 | 构建配置容器 | 包含插件/资源等配置 | 否 |
|  | 构建 | 插件列表容器 | 包含多个 | 否 |
|  | 构建 | 单个插件定义 | 需指定groupId和artifactId | 可选 |
|  | 构建 | 资源文件配置 | 定义路径 | 否 |
|  | 构建 | 测试资源文件配置 | 类似 | 否 |
|  | 构建 | 最终打包文件名 | my-app | 否 |
| 环境配置 |  |  |  |  |
|  | 配置 | 自定义变量容器 | 定义键值对 | 否 |
|  | 属性 | Java版本变量 | 11/17等 | 否 |
|  | 仓库 | 自定义远程仓库列表 | 包含 | 否 |
|  | 仓库 | 自定义插件仓库 | 类似 | 否 |
| 多模块管理 |  |  |  |  |
|  | 模块 | 子模块列表 | 包含多个 | 聚合项目需要 |
...
```
	  ...
	  AI 思考中...
			...
			** [Maven 环境配置](https://www.runoob.com/maven-setup.html)
