...
- **数组**：用于存储固定大小的一组相同类型的数据，定义时指定大小，存储在栈上。 - **切片**：用于引用数组或其他连续内存区域的一部分，大小可动态调整，更灵活，通常引用堆内存。 **数组（Array）：**
...
数组的类型定义为 **[T; N]**，其中 T 是数组元素的类型，N 是数组的长度。 数组的元素可以通过索引访问，索引从 **0** 开始。
...
```
	  ...
	  AI 思考中...
			...
			** [Zig 函数](https://www.runoob.com/zig-fn.html)
			[Zig 结构体和枚举](https://www.runoob.com/zig-struct-enum.html) **
