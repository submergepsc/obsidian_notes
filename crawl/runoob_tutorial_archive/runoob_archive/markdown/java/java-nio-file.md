...
| static List readAllLines(Path path) | 读取文件所有行 |
| static List readAllLines(Path path, Charset cs) | 用指定字符集读取文件所有行 |
| static Stream lines(Path path) | 返回文件中行的流 |
| static Stream lines(Path path, Charset cs) | 用指定字符集返回文件中行的流 |
| static Path write(Path path, byte[] bytes, OpenOption... options) | 将字节写入文件 |
| static Path write(Path path, Iterable lines, OpenOption... options) | 将文本行写入文件 |
| 目录操作 |  |
| static Stream list(Path dir) | 列出目录中的条目 |
| static DirectoryStream newDirectoryStream(Path dir) | 打开目录流 |
...
通过 `Files` 类，Java 提供了更现代、更强大的文件操作 API，推荐在新项目中使用它替代传统的 `java.io.File` 类。
	  ...
	  AI 思考中...
			...
			** [Java File toString() 方法](https://www.runoob.com/java-file-tostring.html)
			[Java java.nio.file.Files copy() 方法](https://www.runoob.com/java-nio-file-copy.html) **
