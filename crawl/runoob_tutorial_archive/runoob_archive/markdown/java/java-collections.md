...
从上面的集合框架图可以看到，Java 集合框架主要包括两种类型的容器，一种是集合（Collection），存储一个元素集合，另一种是图（Map），存储键/值对映射。Collection 接口又有 3 种子类型，List、Set 和 Queue，再下面是一些抽象类，最后是具体实现类，常用的有 [ArrayList](https://www.runoob.com/java-arraylist.html)、[LinkedList](https://www.runoob.com/java-linkedlist.html)、[HashSet](https://www.runoob.com/java-hashset.html)、LinkedHashSet、[HashMap](https://www.runoob.com/java-hashmap.html)、LinkedHashMap 等等。
...
| 序号 | 算法描述 | | --- | --- | | 1 | Collection Algorithms 这里是一个列表中的所有算法实现。 | --- ## 如何使用迭代器 通常情况下，你会希望遍历一个集合中的元素。例如，显示集合中的每个元素。


一般遍历数组都是采用for循环或者增强for，这两个方法也可以用在集合框架，但是还有一种方法是采用迭代器遍历集合框架，它是一个对象，实现了[Iterator](https://www.runoob.com/java-iterator.html) 接口或 ListIterator接口。
...
| 序号 | 迭代器方法描述 |
| --- | --- |
| 1 | 使用 Java Iterator这里通过实例列出 Iterator 和 ListIterator 接口提供的所有方法。 |
...
任何对象加入集合类后，自动转变为Object类型，所以在取出的时候，需要进行强制类型转换。
	  ...
	  AI 思考中...
			...
			** [Java 数据结构](https://www.runoob.com/java-data-structures.html)
			[Java 泛型](https://www.runoob.com/java-generics.html) **