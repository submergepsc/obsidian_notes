...
- Source: https://www.runoob.com/aspnet/razor-vb-variables.html
...
一个变量的名称必须以字母字符开头，并且不能包含空格或者保留字符。** 一个变量可以是一个指定的类型，表示它所存储的数据类型。string 变量存储字符串值（"Welcome to runoob.com"），integer 变量存储数字值（103），date 变量存储日期值，等等。 变量使用 Dim 关键字声明，或通过使用类型（如果您想声明类型）声明，但是 ASP.NET 通常能自动确定数据类型。
...
| >>= | 小于。大于。小于等于。大于等于。 | if iif i>10if iif i>=10 |
...
| AndOR | 逻辑与。逻辑或。 | if ready And clearif ready Or clear |
| AndAlsoorElse | 扩展的逻辑与。扩展的逻辑或。 | if ready AndAlso clearif ready OrElse clear |
...
| ToString() | 转换任何数据类型为字符串。 | myInt=1234myString=myInt.ToString() |
