...
- Source: https://www.runoob.com/vue3/vue3-builtin-attributes.html
...
| 属性 | 核心作用 | 用法/示例 |
| --- | --- | --- |
| is | 解决 HTML 标签语法限制，动态指定渲染的组件/原生标签 |
```
<ul><li is="my-component"></li></ul>
```
 |
| key | 给 Vue 虚拟DOM唯一标识，提升列表渲染效率，保证组件状态正确 |
```
<div v-for="item in list" :key="item.id">{{item}}</div>
```
 |
| ref | 获取DOM元素或组件实例，用于直接操作原生DOM/调用子组件方法 | 模板：ref="名称"脚本：this.$refs.名称
```
<input ref="inputRef">
...
- **类型**: String 或 Object
- **作用**: `is` 属性用于动态地指定当前 `` 组件应该渲染哪一个子组件。通常结合 `` 元素和动态组件的特性一起使用，允许你在运行时基于数据来切换不同的组件。
...
以上代码中，`currentComponent` 是一个数据属性，它决定了 `` 元素要渲染的具体组件。当点击按钮时，调用 `toggleComponent` 方法会切换 `currentComponent` 的值，从而动态改变渲染的组件。

**注意事项：**


- **注意事项**:`is` 属性的值可以是组件的名称字符串，也可以是组件对象。
...
- **类型**: String 或 Number
- **作用**: 在 Vue.js 的列表渲染中，每个 `v-for` 循环中的元素需要有一个唯一的 `key` 属性。`key` 的作用是帮助 Vue 识别每个节点的唯一性，从而高效地更新虚拟 DOM。当 Vue 重新渲染列表时，会根据 `key` 来决定是否重新使用现有 DOM 元素或者重新创建。 ## 实例
```javascript
<ul>
...
- **注意事项**: `key` 应该是唯一且稳定的。稳定的意思是，`key` 不应该随着数据的变化而变化，否则会影响 Vue 的性能和虚拟 DOM 的复用策略。
