...
- Source: https://www.runoob.com/xsl/el-fallback.html
...
The  元素规定了在 XSL 处理器不支持 XSL 元素时，所运行的替代代码。
...
本例本来是要使用一个虚构的  元素来循环遍历每个 "title" 元素。如果 XSL 处理器不支持该元素（它确实不支持），则会使用  元素取而代之：
