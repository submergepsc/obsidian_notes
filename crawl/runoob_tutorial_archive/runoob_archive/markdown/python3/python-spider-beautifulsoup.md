...
- **发送 HTTP 请求**：爬虫通过 HTTP 请求从目标网站获取 HTML 页面，常用的库包括 `[requests](https://www.runoob.com/python-requests.html)`。
...
通常，你会先用爬虫库（如 [requests](https://www.runoob.com/python-requests.html)）获取网页内容:
...
| .is_empty_element | 检查标签是否是空元素（例如 、 等）。 | if tag.is_empty_element: |
...
| find_all(attrs) | 查找具有指定属性的标签。 | tag = soup.find_all(attrs={"href": "http://example.com"}) |
	  ...
	  AI 思考中...
			...
			** [Python selenium 库](https://www.runoob.com/python-selenium.html)
			[Python Scrapy 库](https://www.runoob.com/python-scrapy.html) **
