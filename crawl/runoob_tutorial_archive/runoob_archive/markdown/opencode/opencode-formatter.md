...
- Source: https://www.runoob.com/opencode/opencode-formatter.html

OpenCode 会在每次写入或编辑文件后，**自动**使用对应语言的格式化工具对其进行格式化，确保生成的代码始终符合你项目的代码风格。整个过程在后台静默完成，无需任何手动操作。
...
- 检测被写入或编辑的文件扩展名（如 `.ts`、`.py`、`.go` 等）
- 将扩展名与所有已启用的格式化工具进行匹配
- 对该文件运行匹配到的格式化命令
- 自动将格式化结果写回文件
...
| --- | --- | --- |
| air | .R | air 命令可用 |
| biome | .js .jsx .ts .tsx .html .css .md .json .yaml 及更多 | 项目中存在 biome.json 或 biome.jsonc 配置文件 |
...
**
当多个格式化工具支持同一扩展名时（例如 `.rs` 同时被 `cargofmt` 和 `rustfmt` 支持，`.rb` 同时被 `rubocop` 和 `standardrb` 支持），OpenCode 会根据项目中已安装的工具自动选择。如需明确指定，可通过下方配置禁用不需要的格式化工具。
...
```
	  ...
	  AI 思考中...
			...
			** [OpenCode 自定义命令](https://www.runoob.com/opencode-commands.html)
			[OpenCode 权限配置](https://www.runoob.com/opencode-permissions.html) **
