...
- Source: https://www.runoob.com/opencode/opencode-lsp.html
...
| deno | .ts, .tsx, .js, .jsx, .mjs | 需要 deno 命令可用，且项目中存在 deno.json 或 deno.jsonc（自动检测） |
...
| julials | .jl | 需要安装 julia 及 LanguageServer.jl 包 |
...
| ruby-lsp | .rb, .rake, .gemspec, .ru | 需要 ruby 和 gem 命令可用 |
