...
- Source: https://www.runoob.com/playwright/playwright-first-script.html
...
![](https://www.runoob.com/wp-content/uploads/2025/08/browsers-contexts-pages-880645586-e1690484653870-scaled.png)
...
- `await chromium.launch({ headless: false });`：启动一个 Chromium 浏览器实例。`headless: false` 是一个关键选项，它会**显示浏览器窗口**，让你能看到脚本的执行过程。
- `await browser.newContext();` 和 `await context.newPage();`：这三行代码组合起来，就是打开了一个新的浏览器标签页。
- `await page.goto('https://www.baidu.com');`：**导航**到指定的 URL。这是最常用的方法之一。
- `await page.screenshot({ path: 'baidu_homepage.png' });`：截取当前页面的截图，并将其保存到文件中。
- `await browser.close();`：**关闭**浏览器，释放系统资源。
...
- **文件操作**：你可以将抓取到的数据写入本地文件，以便后续分析。
	  ...
	  AI 思考中...
			...
			** [Playwright 开发环境配置](https://www.runoob.com/playwright-setup.html)
			[Playwright 页面操作基础](https://www.runoob.com/playwright-page-operation-basics.html) **
