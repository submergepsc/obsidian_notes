...
测试代码 *.test.ts describe() / it() expect() 断言 测试结果 ✓ 通过 / ✗ 失败 覆盖率报告 错误堆栈跟踪 单元测试原则 每个测试只验证一件事 Arrange-Act-Assert 结构 测试应该相互独立 --- ## 为什么需要单元测试 单元测试是保证代码质量的重要手段，它可以验证代码的正确性，防止 bug 出现。
