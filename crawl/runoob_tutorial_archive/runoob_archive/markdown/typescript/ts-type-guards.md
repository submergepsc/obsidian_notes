...
typeof typeof === "string" 缩窄为 string 分支 类型已确定 可访问 .length 类型守卫方式 typeof typeof x === "string" 原始类型 instanceof x instanceof Array 类实例 自定义守卫 x is String value is Type in "prop" in x 属性检查 --- ## typeof 类型守卫 typeof 是最常用的类型守卫，用于检查原始类型（string、number、boolean 等）。
