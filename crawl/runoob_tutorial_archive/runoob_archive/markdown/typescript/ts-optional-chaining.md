...
简化 可选链方式（简洁） // 一行搞定 var city = user ?.address ?.city; 安全简洁，自动短路 可选链的三种形式 obj?.prop —— 属性访问 arr?.[0] —— 数组访问 obj?.method() —— 方法调用 --- ## 为什么需要可选链 在 JavaScript/TypeScript 开发中，经常需要访问深层嵌套的对象属性。
