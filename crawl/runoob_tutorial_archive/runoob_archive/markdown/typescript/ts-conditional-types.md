...
执行流程 检查 T 能否赋值给 U → 是 → X | 否 → Y 示例解析 示例 1 IsString string extends string? ✓ → true 示例 2 IsString number extends string? ✗ → false 示例 3 ReturnType fn extends (...args) → R (推导返回类型) --- ## 为什么需要条件类型 在实际的 TypeScript 开发中，我们经常需要根据不同的输入类型返回不同的类型。
...
---
	  ...
	  AI 思考中...
			...
			** [TypeScript 工具类型](https://www.runoob.com/ts-utility-types.html)
