...
字面量类型 var direction: "up" | "down" | "left" | "right" 只能赋这四个值之一 类型安全 赋值错误值时 编译错误！ direction = "upup" 字面量类型种类 字符串字面量 "up" | "down" | "left" 限制特定字符串 数字字面量 1 | 2 | 3 | 100 | 200 限制特定数字 布尔字面量 true | false true 或 false 模板字面量 `on${string}` 动态生成 --- ## 为什么需要字面量类型 在 TypeScript 中，默认的类型如 string 和 number 过于宽泛。
...
进阶：**字面量类型与可辨识联合模式结合，可以实现强大的类型安全。
	  ...
	  AI 思考中...
			...
			** [TypeScript type 别名](https://www.runoob.com/ts-type-alias.html)
			[TypeScript 抽象类](https://www.runoob.com/ts-abstract-class.html) **
