...
- Source: https://www.runoob.com/mysql/mysql-administration.html
...
- `privileges`：所需的权限，如 `ALL PRIVILEGES`、`SELECT`、`INSERT`、`UPDATE`、`DELETE` 等。 - `database_name.*`：表示对某个数据库或表授予权限。`database_name.*` 表示对整个数据库的所有表授予权限，`database_name.table_name` 表示对指定的表授予权限。 - `TO 'username'@'host'`：指定授予权限的用户和主机。 ## 实例
```sql
GRANT ALL PRIVILEGES ON test_db.* TO 'john'@'localhost';
```
 ### 刷新权限 授予或撤销权限后，需要刷新权限使更改生效：
...
![](https://www.runoob.com/wp-content/uploads/2014/03/mysql-admin.gif)
	  ...
	  AI 思考中...
			...
			** [MySQL 安装](https://www.runoob.com/mysql-install.html)
