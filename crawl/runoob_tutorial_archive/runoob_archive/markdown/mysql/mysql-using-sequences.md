...
- Source: https://www.runoob.com/mysql/mysql-using-sequences.html
...
```
 --- ## 重置序列 如果你删除了数据表中的多条记录，并希望对剩下数据的 AUTO_INCREMENT 列进行重新排列，那么你可以通过删除自增的列，然后重新添加来实现。 不过该操作要非常小心，如果在删除的同时又有新记录添加，有可能会出现数据混乱。操作如下所示：
...
```
	  ...
	  AI 思考中...
			...
			** [MySQL 元数据](https://www.runoob.com/mysql-database-info.html)
			[MySQL 处理重复数据](https://www.runoob.com/mysql-handling-duplicates.html) **