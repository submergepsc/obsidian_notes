...
- Source: https://www.runoob.com/ionic/ionic-ion-modal.html
...
| 参数 | 类型 | 详情 |
| --- | --- | --- |
| options | 对象 | 一个选项对象具有一下属性： {object=} 范围 子类的范围。默认：创建一个$rootScope子类。 {string=} 动画 带有显示或隐藏的动画。默认：'slide-in-up' {boolean=} 第一个输入框获取焦点 当显示时，模态窗口的第一个输入元素是否自动获取焦点。默认：false。 {boolean=}backdropClickToClose` 点击背景时是否关闭模态窗口。默认：true。 |
...
[尝试一下 »](https://www.runoob.com/try/tryit.php?filename=ionic_ion-ionicmodal)
