...
- Source: https://www.runoob.com/claude-code/claude-code-slash-commands.html
...
| 项目自定义 Skills 推荐 | /skill-name | 用户创建目录 + SKILL.md | .claude/skills//SKILL.md | 团队共享的可复用工作流，纳入 git；支持 Claude 自动触发 |
| 项目自定义命令 旧格式 | /command | 用户创建 .md 文件 | .claude/commands/ | 团队共享的重复工作流，纳入 git |
| 个人自定义 Skills | /skill-name | 用户创建目录 + SKILL.md | ~/.claude/skills//SKILL.md | 跨项目通用的个人工作流 |
...
| /rename | /rename | 为当前会话命名，便于后续 /resume 找回 | /rename auth-module-refactor | 建议命名规范：功能-操作 |
...
| /btw NEW | /btw | 在不影响主任务上下文的情况下，快速插问一个临时问题，回答不会污染主对话流 | /btw Redis 的默认端口是多少？ | 适合临时查阅而不想干扰当前任务 |
...
| /add-dir | /add-dir | 将额外目录纳入工作范围，适合 monorepo 或跨目录操作 | /add-dir ../packages/shared | 可多次调用添加多个目录 |
...
| ! | ! | 直接执行 Shell 命令，绕过 AI 处理，节省 Token | !git status | 等价于在终端直接运行，但比「帮我查看 git 状态」省 Token |
| @ | @ | 将文件内容注入当前上下文 | @src/api/users.ts | 比手动粘贴代码更精准，支持多文件 |
...
| --- | --- | --- |
| 存储路径 | .claude/skills//SKILL.md | .claude/commands/.md |
...
- **项目 Skill**：只在当前项目生效，团队共享 → 目录：`.claude/skills//`
- **个人 Skill**：所有项目通用，自己用 → 目录：`~/.claude/skills//`
...
| 作用域 | 存储路径 | 共享方式 | 适用场景 |
| --- | --- | --- | --- |
| 项目级 | .claude/commands/.md | 纳入 git，团队共享 | 项目专属工作流（如特定框架的代码生成） |
| 个人级 | ~/.claude/commands/.md | 本地私有 | 通用开发习惯（如个人代码风格检查） |
...
| ~/.claude/skills/ | 个人全局 Skills（跨项目通用，本地私有） | — |
	  ...
	  AI 思考中...
			...
			** [Claude Code 控制与模式](https://www.runoob.com/claude-code-control.html)
