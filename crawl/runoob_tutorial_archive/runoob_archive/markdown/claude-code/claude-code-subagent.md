...
- Source: https://www.runoob.com/claude-code/claude-code-subagent.html
...
**
`bypassPermissions` 只适合完全可信的子代理。另外，子代理会继承父会话的权限模式**——如果主会话开启了 bypass，所有子代理也会跟着 bypass，使用时请格外谨慎。
...
| 范围值 | 存储位置 | 适用场景 |
| --- | --- | --- |
| user | ~/.claude/agent-memory// | 代理的知识适用于所有项目，如通用代码审查规范 |
| project | .claude/agent-memory// | 知识与项目绑定，可通过 git 在团队间共享（推荐默认值） |
| local | .claude/agent-memory-local// | 知识与项目绑定但不提交 git，仅存储个人本地经验 |
...
- 避免为了并行而并行：10 个并行代理处理简单任务反而浪费 Token 和协调成本
	  ...
	  AI 思考中...
			...
			** [Claude Code MCP](https://www.runoob.com/claude-code-mcp.html)
			[Claude Code 插件](https://www.runoob.com/claude-code-plugins.html) **
