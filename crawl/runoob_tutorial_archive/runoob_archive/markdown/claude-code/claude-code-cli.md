...
- Source: https://www.runoob.com/claude-code/claude-code-cli.html

与网页版 Claude 不同，Claude Code 并不是一个单纯的聊天工具，而是一个**具备工程上下文、权限控制和执行能力的开发协作系统**。**因此，它在处理任务时，引入了三种核心的思维模式**：
...
- **写入文件** 会展示代码 Diff，你需要确认后才能写入。
- **执行命令** 例如 `npm test`、`pnpm build`，都会明确询问是否允许。
...
重要提醒：****永远不要在没看 Diff 的情况下直接确认。 Claude 很强，但依然可能产生错误或幻觉。
	  ...
	  AI 思考中...
			...
			** [Claude Code 第一次使用](https://www.runoob.com/claude-code-first-demo.html)
			[Claude Code 基础用法](https://www.runoob.com/claude-code-basic.html) **
