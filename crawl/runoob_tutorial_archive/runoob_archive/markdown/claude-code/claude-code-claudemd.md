...
- Source: https://www.runoob.com/claude-code/claude-code-claudemd.html
...
这是 `CLAUDE.md` 中最高频被参考**的部分。Claude 在执行测试、构建、代码检查等任务时，会优先查找这里定义的命令，避免猜测或使用错误的命令：
...
不能直接引用，但你可以通过 `@文件路径` 语法引用任意 Markdown 文件的内容。如果有跨模块共享的规范，建议提取到独立的文档文件中，再分别在各 `CLAUDE.md` 中用 `@` 引用。
	  ...
	  AI 思考中...
			...
			** [Claude Code 控制 Chrome 浏览器](https://www.runoob.com/claude-code-chrome.html)
