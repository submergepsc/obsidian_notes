...
- Source: https://www.runoob.com/claude-code/claude-code-how-it-work.html
...
**并行多分支开发神器**：**推荐使用 `git worktrees` 为每个分支创建一个独立目录**，这样你就可以同时打开多个 Claude Code 会话，每个分支都有自己完全独立、干净的对话记录，不会互相干扰。
...
- 如果你在多个终端窗口都用 `claude --continue` 恢复同一个会话： 所有消息会**交错混在一起**（就像两个人同时在同一个笔记本上写字）
- 对话会变得混乱
...
- **先探索再实现****先用计划模式**（Shift+Tab 两次）让它分析代码库、给出计划 → 你审核后再让它动手。
