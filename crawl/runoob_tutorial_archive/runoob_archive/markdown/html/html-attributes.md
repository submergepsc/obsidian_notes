...
- Source: https://www.runoob.com/html/html-attributes.html
...
[Link](https://www.runoob.com/../index/index.html)
...
| href | , | 指定链接的目标 URL。 |
| src | , , | 指定外部资源（如图片、脚本、框架）的 URL。 |
| alt |  | 为图像提供替代文本，当图像无法显示时显示。 |
| type | , | 指定输入控件的类型（如 text, password, checkbox 等）。 |
| value | , , | 指定元素的初始值。 |
| disabled | 表单元素 | 禁用元素，使其不可交互。 |
| checked | , | 指定复选框或单选按钮是否被选中。 |
| placeholder | , | 在输入框中显示提示文本。 |
| target | , | 指定链接或表单提交的目标窗口或框架（如 _blank 表示新标签页）。 |
...
**`placeholder`**（用于 `` 和 `` 元素）：在输入框中显示提示文本。
...
**`target`**（用于 `` 和 `` 元素）：指定链接或表单提交的目标窗口或框架。
...
更多标准属性说明： [HTML 标准属性参考手册](https://www.runoob.com/../tags/ref-standardattributes.html).
	  ...
	  AI 思考中...
			...
			** [HTML 元素](https://www.runoob.com/html-elements.html)
