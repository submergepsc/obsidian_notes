...
- Source: https://www.runoob.com/nodejs/nodejs-event.html
...
运行这段代码，1 秒后控制台输出了 **some_event 事件触发**。其原理是 event 对象注册了事件 some_event 的一个监听器，然后我们通过 setTimeout 在 1000 毫秒以后向 event 对象发送事件 some_event，此时会调用some_event 的监听器。 执行结果如下：
...
```javascript
myEmitter.on('data', (value1, value2) => {
  console.log(`Received values: ${value1}, ${value2}`);
});

myEmitter.emit('data', 'Node.js', 'EventEmitter');
// 输出: Received values: Node.js, EventEmitter
```


如果触发的事件没有对应的监听器，EventEmitter 会抛出错误:
...
// 打印: Error event triggered: Error: Something went wrong
```
...
```
	  ...
	  AI 思考中...
			...
			** [Node.js 模块系统](https://www.runoob.com/nodejs-module-system.html)
