...
def split_send(clientSocket, mes: []):
    ...
    while l < n:
        ...
        l += send_len
def send_one_message(clientSocket,mes:str):
    """
    发送一条完整应用层消息。
    方法:消息体前加4字节长度前缀。
    """
    mes_data=mes.encode('utf-8')
    # !I 表示网络字节序的4字节无符号整数
    header=struct.pack('!I',len(mes_data)) 
    split_send(clientSocket,header+mes_data)
if __name__ == '__main__':
    ...
    for mes in messages:
        # 输出发送的消息
        print('client sent mes{}: {}'.format(mes_idx, mes.replace('\0','')))
        # 发送mes
        # split_send(clientSocket, mes.encode('UTF-8'))
        send_one_message(clientSocket,mes)
        # 消息标号加一
        mes_idx += 1
    # 关闭socket
    clientSocket.close()
